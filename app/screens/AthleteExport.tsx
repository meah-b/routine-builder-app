...
export default function AthleteExport() {
    ...
    const fetchData = async () => {
        ...
        try {
            const routinesData = {};
            const vaultRef = collection(doc(firestore_db, 'users', user_uid, 'events', 'vault'),'skills');
            ...
            for (const event of ['bars', 'beam', 'floor']) {
                const routinesRef = collection(doc(firestore_db, 'users', user_uid, 'events', event), 'routines');
    ...
    const handleRoutineSelection = (event, routineId) => {
        ...
        if (updatedRoutinesToExport[event]) {
            if (updatedRoutinesToExport[event].includes(routineId)) {
                updatedRoutinesToExport[event] = updatedRoutinesToExport[event].filter(id => id !== routineId);
            } else {
    ...
    return (
        <LinearGradient colors={colors.gradient} style={styles.outerContainer}>
            <View style={styles.container}>
                ...
                {!loading ? (
                                    ...
                                    eventRoutines.map((routine, index) => (
                                        <TouchableOpacity
                                            ...
                                            <ExportRoutineCard
                                                ...
                                                isSelected={routinesToExport[event] && routinesToExport[event].includes(routine.id)}
                                            />
                                        </TouchableOpacity>
                                    ))
                                ) : (
                    ...
                    <CustomText style={{color: colors.black, fontSize: 25, marginVertical: 243}}>Loading Routine Data...</CustomText>
                )}
