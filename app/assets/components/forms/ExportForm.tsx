...
export default function ExportForm(props: Props){
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            for (const athlete of selectedAthletes) {
                ...
                const vaultRef = collection(doc(firestore_db, 'users', athlete, 'events', 'vault'),'skills');
    ...
    const handleRoutineSelection = (athleteId, event, routineId) => {
        ...
        if (updatedRoutinesToExport[athleteId] && updatedRoutinesToExport[athleteId][event]) {
            if (updatedRoutinesToExport[athleteId][event].includes(routineId)) {
                updatedRoutinesToExport[athleteId][event] = updatedRoutinesToExport[athleteId][event].filter(id => id !== routineId);
            } else {
        ...
        Object.entries(routinesToExport).forEach(([athleteId, athleteData]) => {
            ...
            Object.entries(athleteData).forEach(([event, routineIds]) => {
                ...
                routineIds.forEach(routineId => {
                    if (event === 'vault') {
                        const vaultData = findVaultData(routines[athleteId][event], routineId);
                        if (vaultData) {
                        ...
                        const routineData = findRoutineData(routines[athleteId][event], routineId);
    ...
    return (
        <View style={styles.container}>
            <H1Logo/>
            <CustomText style={styles.h1} bold>Select Routines To Export:</CustomText>
            {!loading ? <ScrollView style={styles.scrollView}>
                {Object.entries(routines).map(([athleteId, athleteData]) => (
                    <View key={athleteId}>
                        <CustomText style={styles.athleteName} bold>{athleteId}</CustomText>
                        {Object.entries(athleteData).map(([event, eventRoutines]) => (
                            <View key={event}>
                                <CustomText bold style={styles.eventName}>{event}</CustomText>
                                {eventRoutines.length > 0 ? (
                                    eventRoutines.map((routine, index) => (
                                        <TouchableOpacity
                                            key={index}
                                            onPress={() => handleRoutineSelection(athleteId, event, routine.id)}
                                            style={styles.routineContainer}>
                                            <ExportRoutineCard
                                                key={routine.id}
                                                name={routine.id}
                                                sv={event === 'vault' ? routine.difficulty.label : (routine.sv  + 10)} 
                                                isSelected={routinesToExport[athleteId] && routinesToExport[athleteId][event] && routinesToExport[athleteId][event].includes(routine.id)}
                                            />
                                        </TouchableOpacity>
                                    ))
                                ) : (
            ...
            </ScrollView> : <CustomText style={{color: colors.black, fontSize: 25, marginVertical: 243}}>Loading Routine Data...</CustomText>}
            <View style={{flexDirection: 'row', gap: 10}}>
                <Button variant="black" onPress={()=>onSubmit()} style={styles.button} title='Reselect Athletes'/>
                <Button variant="black" onPress={()=>exportRoutines()} style={styles.button2} title='Export'/>
            </View>
        </View>
    )
}
